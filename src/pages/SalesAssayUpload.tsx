...
export const SalesAssayUpload: React.FC = () => {
  ...
  const handleSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const allowed = list.filter((f) => {
      ...
      return ext.endsWith('.xlsm') || ext.endsWith('.xls') || ext.endsWith('.xlsx');
    });
  ...
  const handleUpload = async () => {
    if (files.length === 0) {
      setResults([{ fileName: '', success: false, error: '请先选择出厂化验单文件' }]);
      return;
    }
    setUploading(true);
    ...
    for (let i = 0; i < files.length; i++) {
      const file = files[i];
      try {
        const res = await apiClient.uploadSalesAssay(file);
        list.push({
          fileName: file.name,
          success: true,
          message: res.message,
  ...
  return (
    <div className="p-6 max-w-2xl">
      <h1 className="text-2xl font-bold text-slate-900 mb-2">上传出厂化验单</h1>
      <p className="text-slate-600 mb-6">可一次选择多个 Excel 文件批量导入，系统将解析产品名称、客户、日期及每车品位与重量明细。</p>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
        ...
        >
          ...
          <p className="text-sm text-slate-400">支持 .xlsm、.xls、.xlsx，可一次选多个文件</p>
        </div>
